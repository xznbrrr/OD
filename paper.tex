...
\begin{itemize}
    ...
    \item \textbf{Nonconformity index:} The aggregate nonconformity index \citep{miller2013} has no predictive power once firm and year fixed effects are absorbed---\emph{which} dimensions a firm deviates on matters, not the overall level of nonconformity.
\end{itemize}
...
Following \citet{miller2013}, the six strategy variables measure deviation from conformity---the number of standard deviations by which a firm's strategy departs from its 3-digit SIC industry median in a given year:
\begin{equation}
    x_{ikt} = \frac{r_{ikt} - \text{median}_{kt}(r)}{\text{SD}_{kt}(r)},
    \label{eq:deviation}
\end{equation}
where $r_{ikt}$ is the raw strategy ratio for firm $i$, dimension $k$, year $t$. A value of $x_{ikt} = 0$ indicates exact conformity to the industry median; positive values indicate above-median deviation.
...
\midrule
$x_{\text{rnd}}$    & R\&D deviation             & R\&D expenditure / sales \\
$x_{\text{capint}}$ & Capital intensity deviation & Net PPE / sales \\
$x_{\text{lev}}$    & Leverage deviation          & Long-term debt / total assets \\
$x_{\text{adv}}$    & Advertising deviation       & Advertising expenditure / sales \\
$x_{\text{div}}$    & Dividend deviation          & Dividends / earnings \\
$x_{\text{risk}}$   & Unsystematic risk deviation & Residual variance of daily returns \\
\bottomrule
\end{tabular}
\end{table}
...
Table~\ref{tab:correlations} reports pairwise Pearson correlations among the six lagged deviation variables. All correlations are modest in magnitude. The strongest pair is Dividend--Risk ($r \approx -0.35$), indicating that firms conforming more on dividend policy tend to deviate more on unsystematic risk. Leverage--Capital intensity is the next strongest ($r \approx 0.21$).
...
\begin{itemize}
    ...
    \item \textbf{R\&D $\times$ Advertising} is negative in both outcomes, suggesting that high R\&D and high advertising deviation are substitutes rather than complements.
\end{itemize}
...
\midrule
Linear (NCI)          &  0.0004 & (0.0005) & $-$0.0047  & (0.0066) \\
...
\newblock Family firm governance, strategic conformity, and performance: Institutional vs.\ strategic perspectives.
\newblock {\em Organization Science}, 24(1):189--209.
